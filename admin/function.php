...
function add_social_news(){
    global $con;
    if(isset($_POST['add_social_news'])){
         // set the default timezone to use. 
         $con->set_charset("utf8");
         date_default_timezone_set('Asia/Phnom_Penh');
         $title= $_POST['title'];
...
add_social_news();
// @remove_message_from_client
function remove_message_from_client(){
    global $con;
    if(isset($_POST['remove_message_from_client'])){
        $id = $_POST['id'];
        // echo $id;
        $sql_remove = "DELETE FROM `tbl_feedback` WHERE id=$id";
        $result_remove = $con->query($sql_remove);
